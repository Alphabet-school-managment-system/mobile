...
import { LearningMaterialDetail, statusOptions } from "../types";
...
export default function Index() {
  ...
  >();
  const { getGradeLabel, get_speciality_label, formatDate } = useUtil();
  ...
  const handleEdit = () => {
    if (!material) return;
    router.push({
      pathname: "/(app)/(learning-material)/new",
    ...
    setConfirmationModalProps((prev) => ({
      ...
      onOk: () => {
        deleteMaterial(
          ...
          {
            onSuccess: () => {
              router.replace("/(app)/(learning-material)/my-upload");
            },
            onError: () => {},
          },
        );
          ...
          <View className="mt-5 gap-4">
            ...
            <View className="flex-row flex-wrap gap-2">
              <View className="rounded-full bg-white/15 px-3 py-1">
                <Text className="text-white capitalize" variant="bodySmall">
